...
export async function runDailyHealthCheck(): Promise<boolean> {
  ...
  try {
    ...
    // ── 7. Send comprehensive health report email ─────────────────────────
    await sendDailyHealthReport({
      health:             { db: dbOk, ai: aiOk, instagram: igOk, aiProvider, grok: grokHealth.ok, grokDetail: grokHealth.detail },
      storyStatus,
      generatedPosts,
      upcomingPosts:      upcomingPosts.map((p) => ({ title: p.title, scheduledFor: p.scheduledFor, status: p.status })),
      autoGenerated:      generatedPosts.length,
      errors:             [],
      publishedCount24h,
      failedPosts24h,
      commentsReplied24h,
    ...
    console.log(`[DailyHealth] Report sent — published24h: ${publishedCount24h}, failed24h: ${failedPosts24h.length}, rateLimits: ${rateLimitEvents.length}, errors: ${systemErrors.length}, ytPosts24h: ${youtubePosts24h.length}, webhook: ${webhookStatus.active ? "active" : "idle"}`);
    return true;
  } catch (err: any) {
